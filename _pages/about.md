...
Inputs and Outputs: The Data Dynamics
    ...
    Inputs: Crash speed, vehicle front weight, vehicle rear weight.
    Outputs: Deceleration prediction over time or displacement.

Loss Function: The Precision Metric

    bjective: Minimize the discrepancy between CAE predictions and test data.
...
Input Size: 4 (time, initial speed, vehicle front weight, vehicle rear weight).
Output Size: 1 (deceleration at each time step).
Number of Channels: 600.
Dilation Factor: 2^i, where i is the layer number.
Filter Size: 20.
Dropout Rate: 0.05.
Optimizer: Adam.
...
Input Size: 1500 (length of training data).
Output Size: 1 (model bias).
Kernel: Combination of ConstantKernel and MaternKernel.
...
Experimental Data: The Foundation
    ...
    CAE Data: 1009 simulation datasets generated using Latin hypercube sampling.
    Crash Test Data: 11 real-world datasets, limited due to the high costs of physical tests.
...
Key Results: The Breakthrough
    Time-Domain Approach: The Multi-Fidelity TCN significantly improved predictions compared to CAE alone.
    Displacement-Domain Approach: The GPR effectively modeled non-linear dynamics and improved accuracy for unseen configurations.
...
====
  ISO Scores:
    ...
    Test No. 9: 0.734 (0-20 ms), 0.731 (0-40 ms), 0.766 (0-60 ms)
...
====
    ...
    Dataset Expansion: Incorporate more diverse crash scenarios to improve model robustness.
    Hybrid Models: Explore hybrid ML models combining the strengths of different approaches.
    Real-Time Applications: Develop models capable of real-time predictions during crash simulations.
